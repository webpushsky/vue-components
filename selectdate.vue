  ...
  <div>
    <div class="select-date">
      ...
      <div v-for="(item, index) in pullDate" :key='index'>
        ...
        <ul>
          <li v-for="(day, key) in item.date" :class="day.classname" @click="(day.classname==='day'? dayClickEvent(day): null)" :key='key'>{{day.day}}
            <i v-for="(tag, keys) in day.tags" :class="tag[0]" :key='keys'>
...
export default {
  ...
  methods: {
    ...
    getAll (start, end) {
      ...
      // 对本月一号之前的周几补全
        dataObject[sd].date.push({year: '', month: '', day: '', week: w})
      ...
      for (let i = sd; i <= ed;) {
        ...
        // 当月第一天;--作为每个月的唯一标示
        if (moment(i).format('x') === moment(moment(i).startOf('month').format('YYYY-MM-DD')).format('x') && i !== sd) {
          // 如果是当月的第一天,添加下个月的数据
          console.log(i, sd)
          const op = {
          ...
          for (let w = 1; w < moment(i).weekday(); w++) {
            // 对本月一号之前的周几补全。
            op.date.push({year: '', month: '', day: '', week: w})
            // 如果当前月份没有存储当前天数用的数组,就创建一个空数组，如果有，就向里面添加一个空对象; (空对象是用来占位置的，用来填充月份前面的空白)
          }
          dataObject[i] = op
        }
